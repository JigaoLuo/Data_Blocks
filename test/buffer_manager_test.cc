...
#ifdef DEBUG
// NOLINTNEXTLINE
TEST(BufferManagerTest, AlignmentTest) {
  /// New 10000 BufferManager object
  /// assert in the BufferManager constructor to check to data alignment
  for (size_t i = 0; i < 10000; ++i) {
      imlab::unique_ptr_aligned<char[]> loaded_pages(static_cast<char*>(imlab::aligned_malloc(32, 1024 * 10)), &imlab::aligned_free_wrapper);
      imlab::BufferManager buffer_manager(1024, 10, std::move(loaded_pages));
  }
}
...
TEST(BufferManagerTest, PersistentRestart) {
    auto buffer_manager = std::make_unique<imlab::BufferManager>(1024, 10, imlab::unique_ptr_aligned<char[]>(static_cast<char*>(imlab::aligned_malloc(32, 1024 * 10)), &imlab::aligned_free_wrapper));
    ...
    buffer_manager = std::make_unique<imlab::BufferManager>(1024, 10, imlab::unique_ptr_aligned<char[]>(static_cast<char*>(imlab::aligned_malloc(32, 1024 * 10)), &imlab::aligned_free_wrapper));
    for (uint16_t segment = 0; segment < 3; ++segment) {
        for (uint64_t segment_page = 0; segment_page < 10; ++segment_page) {
            uint64_t page_id = (static_cast<uint64_t>(segment) << 48) | segment_page;
...
TEST(BufferManagerTest, BufferFull) {
    imlab::unique_ptr_aligned<char[]> loaded_pages(static_cast<char*>(imlab::aligned_malloc(32, 1024 * 10)), &imlab::aligned_free_wrapper);
    imlab::BufferManager buffer_manager(1024, 10, std::move(loaded_pages));
    std::vector<imlab::BufferFrame*> pages;
    pages.reserve(10);
    for (uint64_t i = 1; i < 11; ++i) {
        auto& page = buffer_manager.fix_page(i, false);
        pages.push_back(&page);
    }
    EXPECT_THROW(buffer_manager.fix_page(11, false), imlab::buffer_full_error);
    for (auto* page : pages) {
        buffer_manager.unfix_page(*page, false);
    }
}
    ...
    for (size_t i = 0; i < 4; ++i) {
        threads.emplace_back([i, &buffer_manager, &num_buffer_full, &finished_threads] {
            ...
            for (size_t j = 0; j < 4; ++j) {
                ...
                } catch (const imlab::buffer_full_error&) {
                    ++num_buffer_full;
                }
            }
            ++finished_threads;
