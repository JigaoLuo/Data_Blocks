...
// ---------------------------------------------------------------------------
#ifndef INCLUDE_IMLAB_UTIL_TYPES_H_
#define INCLUDE_IMLAB_UTIL_TYPES_H_
//---------------------------------------------------------------------------
#include <algorithm>
#include <cassert>
#include <cstdint>
...
//---------------------------------------------------------------------------
template <unsigned kMaxLen> struct LengthIndicator {
...
template <unsigned kMaxLen> class Varchar {
 public:
    /// The length
...
//---------------------------------------------------------------------------
// Hash
template <unsigned kMaxLen> uint64_t Varchar<kMaxLen>::hash() const {
    unsigned result = 0;
...
//---------------------------------------------------------------------------
// Comparison
template <unsigned kMaxLen> bool Varchar<kMaxLen>::operator<(const Varchar &other) const {
...
template <unsigned kMaxLen> class Char {
 public:
    /// The length
    typename LengthIndicator<kMaxLen>::type len;
    /// The data
    ...
    static Char<kMaxLen> castString(const char *str, uint32_t strLen) {
        ...
        assert(strLen <= kMaxLen); Char<kMaxLen> result;
        result.len = strLen;
        memcpy(result.value, str, strLen); return result;
    }
};
//---------------------------------------------------------------------------
// Hash
template <unsigned kMaxLen> uint64_t Char<kMaxLen>::hash() const {
    unsigned result = 0;
    for (unsigned index = 0; index < len; index++)
        result = ((result << 5) | (result >> 27)) ^ (static_cast<unsigned char>(value[index]));
    return result;
...
//---------------------------------------------------------------------------
// Comparison
template <unsigned kMaxLen> bool Char<kMaxLen>::operator<(const Char &other) const {
    int c = memcmp(value, other.value, min(len, other.len));
    if (c < 0) return true;
    if (c > 0) return false;
    return len < other.len;
...
template <unsigned len, unsigned precision> class Numeric {
 public:
    ...
    template <unsigned l> Numeric<l, precision> castS() const {
        Numeric<l, precision> r; r.value = value; return r;
    }
    /// Cast
    template <unsigned l> Numeric<l, precision + 1> castP1() const {
        Numeric<l, precision + 1> r; r.value = value * 10; return r;
    }
    /// Cast
    Numeric<len, precision + 2> castP2() const {
        Numeric<len, precision + 2> r; r.value = value * 100; return r;
    }
    /// Cast
    ...
    template <unsigned l> Numeric<l, precision - 2> castM2() const {
        Numeric<l, precision - 2> r; r.value = value / 100; return r;
    }
    /// Build a number
    static Numeric<len, precision> buildRaw(int64_t v) { Numeric r; r.value = v; return r; }
    /// Cast
...
//---------------------------------------------------------------------------
#endif  // INCLUDE_IMLAB_UTIL_TYPES_H_
